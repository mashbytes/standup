...
struct FakeTasks {
    ...
    static func todo() -> Task {
        ...
        return Task(id: identifier, title: identifier, status: .todo(0))
    }
    
    static func wip() -> Task {
        let identifier = FakeTasks.IdentifierCache.nextWIP()
        return Task(id: identifier, title: identifier, status: .wip(0))
    }
    
    static func doneToday() -> Task {
        let identifier = FakeTasks.IdentifierCache.nextDone()
        return Task(id: identifier, title: identifier, status: .done(today))
    }
    
    static func doneYesterday() -> Task {
        let identifier = FakeTasks.IdentifierCache.nextDone()
        return Task(id: identifier, title: identifier, status: .done(yesterday))
    }
